CREATE TABLE movie_db.publication (name VARCHAR(100) PRIMARY KEY, avatar VARCHAR(21));
CREATE TABLE movie_db.reviewer (name VARCHAR(100) PRIMARY KEY, avatar VARCHAR(100), publication VARCHAR(100), FOREIGN KEY (publication) REFERENCES publication(name) ON DELETE CASCADE);
CREATE TABLE movie_db.moviereview (title VARCHAR(100) PRIMARY KEY, `release` VARCHAR(100), score INTEGER, reviewer VARCHAR(100), FOREIGN KEY (reviewer) REFERENCES reviewer(name) ON DELETE CASCADE);
